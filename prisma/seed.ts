...
async function main() {
  ...
  const basicWhatsappPackage = await prisma.whatsappApiPackage.create({
    data: {
      ...
      priceMonth_idr: 150000,
      priceMonth_usd: 10,
      priceYear_idr: 1500000,
      priceYear_usd: 100,
      maxSession: 1
    }
  });
  ...
  const enterpriseWhatsappPackage = await prisma.whatsappApiPackage.create({
    data: {
      ...
      priceMonth_idr: 1500000,
      priceMonth_usd: 100,
      priceYear_idr: 15000000,
      priceYear_usd: 1000,
      maxSession: 99
    }
  });
  ...
  console.log(`⚙️  Duitku Config: 1`);
  console.log(`📱 System Sessions: 2`);
  ...
  console.log('💬 Creating system WhatsApp sessions...');
  ...
  const adminSession = await prisma.whatsAppSession.create({
    data: {
      ...
      connected: false,
      status: 'disconnected',
      message: 'System session for admin notifications and OTP'
    }
  });
  ...
  const userSession = await prisma.whatsAppSession.create({
    data: {
      ...
      connected: false,
      status: 'disconnected',
      message: 'System session for user OTP and notifications'
    }
  });

  // 14. Create Sample Vouchers
  console.log('🎫 Creating sample vouchers...');

  console.log('\n💬 System WhatsApp Sessions:');
  console.log(`   • Admin Session: ${adminSession.sessionId} (${adminSession.token})`);
  console.log(`   • User Session: ${userSession.sessionId} (${userSession.token})`);
}
