...
export default function PaymentDashboardPage() {
  ...
  const loadPayments = useCallback(async (page = 1) => {
    try {
      ...
      // Use SessionManager like other customer pages
      ...
      const response = await fetch(`/api/customer/payment?page=${page}&limit=${pagination.limit}`, {
        method: 'GET',
        headers: {
    ...
    try {
      ...
      if (!token) {
        toast({
          variant: "destructive",
          title: "Authentication Required",
          description: "Please login to cancel payment."
        })
        return
      }
      
      const response = await fetch(`/api/customer/payment/${paymentId}/cancel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    try {
      ...
      if (!token) {
        toast({
          variant: "destructive",
          title: "Authentication Required",
          description: "Please login to export payment history."
        })
        return
      }
      ...
      const response = await fetch(`/api/customer/payment/export?${params.toString()}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      })
      ...
      if (format === 'csv') {
        ...
        link.href = url
        link.download = `payment-history-${formatDate(new Date(), 'yyyy-MM-dd')}.csv`
        ...
        if (data.success) {
          ...
          const link = document.createElement('a')
          link.href = url
          link.download = `payment-history-${formatDate(new Date(), 'yyyy-MM-dd')}.json`
          document.body.appendChild(link)
          link.click()
          document.body.removeChild(link)
          URL.revokeObjectURL(url)
          ...
          toast({
            title: "Export Completed",
            description: "Payment history has been exported to JSON"
          })
        } else {
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex-1 space-y-2">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <Button
                                ...
                                onClick={() => router.push(`/dashboard/payment/${payment.id}`)}
                              >
                        ...
                        <div className="ml-12 flex items-center gap-4 text-sm text-muted-foreground">
                          <span className="font-medium">{formatCurrency(payment.amount, payment.transaction.currency || 'IDR')}</span>
                          <span>•</span>
