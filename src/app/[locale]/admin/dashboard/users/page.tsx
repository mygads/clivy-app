...
export default function UsersPage() {
  ...
  const [currentUserId, setCurrentUserId] = useState<string | null>(null);const [formData, setFormData] = useState<UserFormData>({
    name: "",
    email: "",
    phone: "",
    password: "",
      ...
      if (!currentUserId) {
        try {
          ...
          if (sessionRes.ok) {
            ...
            if (sessionData.success && sessionData.data?.user?.id) {
              setCurrentUserId(sessionData.data.user.id);
            }
          }
        } catch (error) {
  ...
  }, [limit, offset, search, roleFilter, currentUserId]);
  ...
  const handleToggleActive = async (user: User) => {
    if (toggleLoading === user.id) return; // Prevent multiple clicks
    
    // Prevent admin from deactivating their own account
    if (currentUserId && user.id === currentUserId && user.isActive) {
      toast.error("You cannot deactivate your own account");
      return;
    }
    
    // Show confirmation dialog for deactivating other admins
    if (user.role === 'admin' && user.isActive && user.id !== currentUserId) {
      const confirmDeactivate = window.confirm(
        `Are you sure you want to deactivate admin user "${user.name}"? This will revoke their administrative privileges.`
      );
      ...
      {!loading && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            ...
            <CardContent className="pt-0">
              <div className="text-lg sm:text-2xl font-bold text-orange-600">{stats.totalCustomers}</div>
              <p className="text-xs text-muted-foreground">
                Customer accounts
              </p>
            </CardContent>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 sm:pb-3">
              ...
              <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-lg bg-red-500/10 flex items-center justify-center">
                <Crown className="h-3 w-3 sm:h-4 sm:w-4 text-red-600 dark:text-red-400" />
              </div>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="text-lg sm:text-2xl font-bold text-red-600">{stats.totalAdmins}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 sm:pb-3">
              <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">Verified</CardTitle>
            ...
            <CardContent className="pt-0">
              <div className="text-lg sm:text-2xl font-bold text-green-600">{stats.verifiedUsers}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 sm:pb-3">
              <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">Recent (7d)</CardTitle>
            ...
            <CardContent className="pt-0">
              <div className="text-lg sm:text-2xl font-bold text-purple-600">{stats.recentRegistrations}</div>
              <p className="text-xs text-muted-foreground">
                New registrations
              </p>
            </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base sm:text-lg md:text-xl font-semibold">Users</CardTitle>
          </div>
        ...
        <CardContent>
          {loading ? (
            <div className="flex items-center justify-center p-8">
              ...
              <span className="ml-2 text-xs sm:text-sm">Loading users...</span>
            </div>
              ...
              <table className="w-full min-w-[900px]">
                <thead>
                  <tr className="border-b">
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">User</th>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">Contact</th>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">Role</th>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">Status</th>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">Activity</th>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">Created</th>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-medium">Actions</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <Switch
                            ...
                            disabled={toggleLoading === user.id || (currentUserId === user.id && user.isActive)}
