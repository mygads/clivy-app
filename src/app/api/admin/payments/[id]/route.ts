...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'approve' && updatedTransaction?.status === 'in_progress') {
      ...
      // Send payment success notifications after admin approval
      setImmediate(async () => {
        try {
          console.log(`[ADMIN_APPROVAL_NOTIFICATION] Sending payment success notifications for admin-approved payment ${id}`);

          if (!payment.transaction) {
            console.error(`[ADMIN_APPROVAL_NOTIFICATION] No transaction found for payment ${id}`);
            return;
          }

          // Get user phone from database since it's not included in the current query
          const userWithPhone = await prisma.user.findUnique({
            where: { id: payment.transaction.user?.id },
            select: { phone: true }
          });
          ...
          const notificationResults = await PaymentNotificationService.sendPaymentSuccessNotifications({
            ...
            transactionId: payment.transaction.id,
            customerName: payment.transaction.user?.name || 'Customer',
            customerEmail: payment.transaction.user?.email || undefined,
            customerPhone: userWithPhone?.phone || undefined,
            items,
            subtotal: Number(payment.transaction.amount || 0),
            discountAmount: Number(payment.transaction.discountAmount || 0),
            serviceFeeAmount: Number(payment.transaction.serviceFeeAmount || 0),
            finalAmount: Number(payment.transaction.finalAmount || payment.transaction.amount || 0),
            currency: payment.transaction.currency,
            paymentMethod: payment.method || 'manual',
            paymentMethodName: payment.method.includes('manual') ? 'Manual Bank Transfer' : payment.method,
            paymentDate: updatedPayment.paymentDate || new Date(),
            orderDate: payment.transaction.createdAt
          })

          console.log(`[ADMIN_APPROVAL_NOTIFICATION] Payment success notifications sent for payment ${id}:`, {
            whatsappSent: notificationResults.whatsappSent,
            emailSent: notificationResults.emailSent
          })
        } catch (notificationError) {
          console.error(`[ADMIN_APPROVAL_NOTIFICATION] Error sending payment success notifications for payment ${id}:`, notificationError)
        }
      });
    }    return withCORS(NextResponse.json({
      success: true,
