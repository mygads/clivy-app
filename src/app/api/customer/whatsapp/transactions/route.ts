...
export async function GET(request: NextRequest) {
  try {
    ...
    const [legacyTransactions, newTransactions, totalLegacy, totalNew] = await Promise.all([
      // Legacy system - ServicesWhatsappCustomers (no transaction relation)
      ...
      // Count for legacy (no transaction relation, use subscription status)
      prisma.servicesWhatsappCustomers.count({
        where: {
          customerId: userId,
          // Legacy system doesn't have transaction relation
          // Use subscription status instead if needed
    ...
    // Format legacy transactions (no transaction relation available)
    const formattedLegacyTransactions = legacyTransactions.map(sub => ({
      id: sub.id,
      transactionId: null, // Legacy system doesn't have transaction relation
      packageName: sub.package.name,
      packageDescription: sub.package.description,
      maxSessions: sub.package.maxSession,
      amount: null, // No transaction data available
      status: 'legacy', // Legacy status
      paymentMethod: null, // No payment data available
      purchaseDate: sub.activatedAt, // Use activatedAt as purchase date
      startDate: sub.activatedAt,
