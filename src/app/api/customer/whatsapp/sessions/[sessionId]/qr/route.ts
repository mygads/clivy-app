...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Handle "already logged in" case - this is not an error, it's a valid state
      if (response.status === 500 && responseData.error === "already logged in") {
        // Also update session status in database to reflect current state
        await prisma.whatsAppSession.update({
          ...
          data: {
            ...
            loggedIn: true,
            status: 'connected',
            qrcode: null, // Clear QR code since not needed
            updatedAt: new Date()
          }
        });
        ...
        return withCORS(NextResponse.json({
          ...
          data: {
            ...
            QRCode: "", // No QR needed since already logged in
    ...
    console.error("[CUSTOMER_WHATSAPP_SESSION_QR]", error);
    return withCORS(NextResponse.json(
