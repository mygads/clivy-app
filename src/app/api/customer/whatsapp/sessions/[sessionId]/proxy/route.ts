...
// POST /api/customer/whatsapp/sessions/[sessionId]/proxy - Update proxy configuration
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[PROXY_POST] External service error:`, errorData);
      return withCORS(NextResponse.json({ 
        success: false, 
    ...
    if (!data.success) {
      console.error(`[PROXY_POST] External service returned error:`, data);
      return withCORS(NextResponse.json({ 
        success: false, 
    ...
    console.error("[PROXY_POST]", error);
    return withCORS(NextResponse.json(
...
// DELETE /api/customer/whatsapp/sessions/[sessionId]/proxy - Delete proxy configuration
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error(`[PROXY_DELETE] External service error:`, errorData);
      return withCORS(NextResponse.json({ 
        success: false, 
        error: errorData.details || `WhatsApp service error: ${response.status}` 
      }, { status: 500 }));
    ...
    if (!data.success) {
      console.error(`[PROXY_DELETE] External service returned error:`, data);
      return withCORS(NextResponse.json({ 
        success: false, 
        error: data.details || 'WhatsApp service returned error' 
      }, { status: 500 }));
    ...
    console.error("[PROXY_DELETE]", error);
    return withCORS(NextResponse.json(
