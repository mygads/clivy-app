...
export async function POST(request: NextRequest) {
  try {
    ...
    const { items, packages, addons, whatsapp, currency: requestedCurrency, referenceLink, voucherCode, notes } = validation.data;
    
    // Auto-detect currency based on IP if not provided
    const currency = requestedCurrency || await detectCurrency(request);
    
    console.log(`[CHECKOUT] Auto-detected currency: ${currency.toUpperCase()} for user ${userAuth.id}`);    // Convert items array to packages/addons format if provided
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // 4. Determine transaction type based on included items
      ...
      const transaction = await PaymentExpirationService.createTransactionWithExpiration({
        ...
        totalAfterDiscount: Number(finalTotal), // Use finalTotal instead of totalAfterDiscount
