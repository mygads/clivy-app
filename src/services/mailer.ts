 ...
 */
export async function sendEmail(options: MailOptions): Promise<MailerResponse> {
    try {
        if (process.env.NODE_ENV === 'production' || (process.env.EMAIL_SERVER_USER && process.env.EMAIL_SERVER_PASSWORD)) {
            ...
            console.log(`Mailer: DEVELOPMENT MODE: Email NOT SENT to ${options.to}. Subject: ${options.subject}`);
            return { success: true, message: 'Development mode: Email not sent.' };
        }
    } catch (error: unknown) {
        console.error(`Mailer: Failed to send email to ${options.to}. Error:`, error);
        return { 
            success: false, 
