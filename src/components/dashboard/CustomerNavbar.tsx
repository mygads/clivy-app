...
export function DashboardNavbar({ cartOpen, setCartOpen }: DashboardNavbarProps) {
  ...
  const switchLanguage = (newLocale: string) => {
    handleLocaleChange(newLocale as 'id' | 'en', pathname)
  }
  ...
  return (
    <header className="sticky top-0 z-50 flex h-14 shrink-0 items-center gap-2 border-b border-border bg-white dark:bg-black px-4 shadow-sm">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="flex items-center h-8 gap-0 px-3 rounded-lg hover:bg-accent/80 bg-accent/50 transition-colors uppercase"
            >
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-8 gap-2 px-2 rounded-md hover:bg-accent transition-colors">
              <Avatar className="h-6 w-6 ring-2 ring-primary/30 dark:ring-primary/30">
                <AvatarImage src={userDisplayData.avatar} alt={userDisplayData.name || "User"} />
