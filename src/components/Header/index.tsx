...
import { handleLocaleChange } from "@/lib/locale-management"
...
const Header = () => {
  ...
  const [hoverTimeouts, setHoverTimeouts] = useState<Record<number, NodeJS.Timeout | undefined>>({})
  ...
  const handleMouseLeave = (index: number) => {
    ...
    if (clickedIndex === -1) {
      // Add longer delay before closing to allow mouse movement
      const timeoutId = setTimeout(() => {
        setOpenIndex(-1)
      }, 300) // Increased to 800ms delay for better UX and prevent flickering
  ...
  const handleDropdownMouseEnter = (index: number) => {
    ...
    Object.keys(hoverTimeouts).forEach((key) => {
      const timeoutId = hoverTimeouts[parseInt(key)]
      if (timeoutId) {
    ...
    // Ensure the current dropdown stays open
    if (clickedIndex === -1) {
      setOpenIndex(index)
    }
  }

  // Enhanced group hover handlers to ensure dropdown stays open
  const handleGroupMouseEnter = (index: number) => {
    // Clear ALL timeouts to prevent interference between menus
    Object.keys(hoverTimeouts).forEach((key) => {
      const timeoutId = hoverTimeouts[parseInt(key)]
      if (timeoutId) {
        clearTimeout(timeoutId)
  ...
  const handleGroupMouseLeave = (index: number) => {
    // Add a small delay before starting close timer to prevent immediate closing
    // when mouse moves between menu items
    setTimeout(() => {
      handleMouseLeave(index)
    }, 50) // Very short delay to allow mouse movement between items
  }
  ...
  const handleLocaleSwitch = (targetLocale: string) => {
    // Use the proper locale management system to store preference and redirect
    handleLocaleChange(targetLocale as 'id' | 'en', usePathName)
  }
  ...
  // Tutup mobile navbar jika klik di luar
  const handleNavbarClickOutside = useCallback((event: MouseEvent) => {
    if (navbarRef.current && !navbarRef.current.contains(event.target as Node)) {
      // Pastikan klik tidak pada hamburger button
  ...
  useEffect(() => {
    if (navbarOpen) {
      document.addEventListener("mousedown", handleNavbarClickOutside)
    } else {
      document.removeEventListener("mousedown", handleNavbarClickOutside)
    }
    return () => {
      document.removeEventListener("mousedown", handleNavbarClickOutside)
    }
  }, [navbarOpen, handleNavbarClickOutside])
  ...
  return (
    <>
                  ...
                  <ul className="block lg:flex lg:space-x-8">                    
                    {menuData.slice(0, 5).map((menuItem, index) => (
                      <li
                        ...
                        onMouseEnter={() => (menuItem.submenu || menuItem.megaMenu) && handleGroupMouseEnter(index)}
                        onMouseLeave={() => (menuItem.submenu || menuItem.megaMenu) && handleGroupMouseLeave(index)}
                      >                        {menuItem.path ? (
                          <Link
                            href={menuItem.path || "#"}
                            onClick={() => {
                            ...
                            className={`flex py-2 text-base lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 font-medium transition-all duration-300 relative ${
                              menuItem.path && isPathActive(menuItem.path)
                            ...
                            <p
                              ...
                              className={`flex cursor-pointer items-center justify-between py-2 text-base lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 font-medium transition-all duration-300 relative ${
                                ...
                                (menuItem.submenu && menuItem.submenu.some(sub => sub.path && isPathActive(sub.path))) ||
                                (menuItem.megaMenu && menuItem.megaMenu.some(cat => cat.items.some(item => item.path && isPathActive(item.path))))
                            ...
                            {(menuItem.submenu || menuItem.megaMenu) && (
                              <div 
                                className="hidden lg:block absolute top-full left-0 w-full h-2 bg-transparent pointer-events-auto"
                            ...
                            {menuItem.submenu && (
                              <div
                              onMouseEnter={() => handleDropdownMouseEnter(index)}
                              className={`submenu relative left-0 top-full rounded-sm bg-white transition-[opacity] duration-200 dark:bg-bg-color-dark lg:invisible lg:absolute lg:top-[100%] lg:block lg:w-[250px] lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:opacity-100 ${
                                openIndex === index ? "block max-h-[60vh] overflow-y-auto lg:max-h-[75vh] lg:overflow-y-auto xl:max-h-none xl:overflow-visible lg:!visible lg:!opacity-100" : "hidden"
                              }`}
                              style={{
                                pointerEvents: openIndex === index ? "auto" : "none",
                                paddingTop: 16,
                                paddingBottom: 16,
                                marginTop: -8, // Negative margin to close gap
                                zIndex: 30,
                              }}
                              >
                              {menuItem.submenu.map((submenuItem, idx) => {
                                const Icon = submenuItem.icon ? (FiIcons as any)[submenuItem.icon] : null
                                ...
                                <div key={idx}>
                                  ...
                                  <Link
                                    ...
                                    className={`flex items-center gap-3 rounded-lg py-2.5 px-3 text-sm transition-all border mb-1 ${
                                      submenuItem.path && isPathActive(submenuItem.path)
                            ...
                            {menuItem.megaMenu && (
                              <div
                              onMouseEnter={() => handleDropdownMouseEnter(index)}
                              className={`mega-menu relative left-0 top-full rounded-sm bg-white transition-[opacity] duration-200 dark:bg-bg-color-dark lg:invisible lg:absolute lg:top-[100%] lg:block lg:w-[750px] lg:p-6 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:opacity-100 ${
                                openIndex === index ? "block max-h-[60vh] overflow-y-auto lg:max-h-[75vh] lg:overflow-y-auto xl:max-h-none xl:overflow-visible lg:!visible lg:!opacity-100" : "hidden"
                              }`}
                              style={{ 
                                pointerEvents: openIndex === index ? "auto" : "none",
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {menuItem.megaMenu.map((category, catIdx) => (
                                  ...
                                  {category.items.map((item, itemIdx) => {
                                    const Icon = item.icon ? (FiIcons as any)[item.icon] : null
                                    const isComingSoon = item.title === "whatsappChatbot" || item.title === "whatsappTeamInbox"
                                    return (
                                    <div key={itemIdx} className="relative">
                                      ...
                                      className={`relative flex items-start gap-3 rounded-lg py-2 px-3 text-sm transition-all border ${
                                        isComingSoon
                                          ? "text-gray-400 dark:text-gray-500 border-transparent cursor-not-allowed pointer-events-none"
                                          : item.path && isPathActive(item.path)
                                          ? "bg-primary/20 text-primary border-primary/50 dark:bg-primary/30 dark:text-white dark:border-primary/50 font-semibold"
                                          : "text-dark hover:bg-primary/10 hover:text-primary dark:text-white dark:hover:text-white dark:hover:bg-primary/20 border-transparent hover:border-primary/30"
                                      }`}>
                                      {Icon && <Icon className={`w-5 h-5 mt-0.5 flex-shrink-0 ${isComingSoon ? "text-gray-400 dark:text-gray-500" : "text-primary dark:text-white"}`} />}
                                      <div className="flex-1">
                                        {isComingSoon && (
                                          <div className="mb-2">
                                            
                                            <AnimatedGradientText 
                                              className="text-[10px] font-medium px-2 py-1 rounded-full whitespace-nowrap bg-white dark:bg-gray-800 border-2 shadow-sm"
                                              colorFrom="#1d4ed8"
                                              colorTo="#dc2626"
                                              speed={0.8}
                                            >
                                        ...
                                        <div className="font-medium">
                                          <span className={isComingSoon ? "text-gray-400 dark:text-gray-500" : ""}>{t(item.title)}</span>
                                        </div>
                                        {item.desc && (
                                        <div className={`text-xs mt-0.5 leading-tight ${isComingSoon ? "text-gray-400 dark:text-gray-600" : "text-gray-500 dark:text-white/70"}`}>
                                          {t(item.desc)}
                                        </div>
                                        )}
                                      </div>
      ...
      {/* Cart sidebar outside header for proper viewport positioning */}
