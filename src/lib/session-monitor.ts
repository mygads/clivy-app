      ...
      where: {
        OR: [
          ...
          },          // Include sessions that are 'stopped' but not yet terminated from server
          {
            ...
            // (status 'stopped' means they're not terminated yet - terminated sessions have status 'terminated')
          }
        ],
    ...
    console.log(`[SESSION_MONITOR] Found ${staleSessions.length} stale sessions to check`);    for (const session of staleSessions) {
      try {
        ...
        if (session.status === 'stopped') {
          console.log(`[SESSION_MONITOR] Session ${session.sessionId} stopped in DB - attempting server termination`);
          ...
          try {
            ...
            // Update status to terminated to prevent future redundant calls
