 ...
 * Updated to properly handle 401 vs 500 errors
 */
export async function validateAuthWithServer(): Promise<AuthResponse> {
  try {
    ...
    if (response.status === 401) {
      ...
      console.warn('[CLIENT_AUTH] Token invalid or expired, clearing session');
      removeUserSession();
      return {
        success: false,
        authenticated: false,
      ...
      // Server error, don't clear session - just return error
      console.error('[CLIENT_AUTH] Server error during auth validation:', data.error);
      return {
        success: false,
        authenticated: false,
        session: null,
    ...
    } else if (!response.ok || !data.success || !data.authenticated) {
      // Other client errors (4xx), clear session
      console.warn('[CLIENT_AUTH] Authentication failed with status:', response.status);
      removeUserSession();
      return {
        success: false,
        authenticated: false,
    ...
    // Network error or other issues - don't clear session
    return {
      success: false,
      authenticated: false,
      session: null,
 ...
 */
export function getSessionTimeRemainingMinutes(): number {
 ...
 */
export function isSessionExpiringSoon(thresholdMinutes: number = 30): boolean {
  const remainingMinutes = getSessionTimeRemainingMinutes();
  return remainingMinutes > 0 && remainingMinutes <= thresholdMinutes;
}

/**
 * Handle authentication errors from API responses
 * Only clears session for 401 (Unauthorized), not for 500 (Server Error)
 */
export function handleAuthError(statusCode?: number): void {
  if (statusCode === 401) {
    console.warn('[CLIENT_AUTH] Authentication error (401) detected, clearing session');
    ...
    console.error('[CLIENT_AUTH] Server error (500) detected, keeping session');
    // Don't clear session for server errors
  } else {
    console.warn('[CLIENT_AUTH] Unknown authentication error detected, clearing session');
    // For unknown errors, clear session to be safe
    if (typeof window !== 'undefined') {
      import('./storage').then(({ SessionManager }) => {
        SessionManager.clearSession();
      });
